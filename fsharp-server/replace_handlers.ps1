$content = Get-Content Handlers.fs -Raw

# Replace wrapCytoscapeHtml
$pattern = 'let private wrapCytoscapeHtml \(title: string\) \(data: string\) \(enableSave: bool\) : string =.*?</html>""" title data saveScript'
$replacement = @'
...
# Replace wrapMermaidHtml
$pattern2 = 'let private wrapMermaidHtml \(title: string\) \(diagram: string\) : string =.*?</html>""" title title diagram'
$replacement2 = @'
...
[System.IO.File]::WriteAllText('Handlers.fs', $content)
Write-Host "Done"
