...
module Routes =
    ...
    let createHandlers (registry: ElementRegistry) (assets: DiagramAssetConfig) (webConfig: WebUiConfig) (loggerFactory: ILoggerFactory) : HttpHandler =
        ...
        choose [
            ...
            route "/architecture" >=> Handlers.architectureIndexHandler registry webConfig logger
            route "/governance" >=> Handlers.governanceIndexHandler registry webConfig logger
            routef "/governance/%s" (fun slug -> Handlers.governanceDocHandler slug registry webConfig logger)
            route "/management-system" >=> Handlers.governanceIndexHandler registry webConfig logger
            routef "/management-system/%s" (fun slug -> Handlers.governanceDocHandler slug registry webConfig logger)
            ...
            routef "/diagrams/governance/%s" (fun slug -> Handlers.governanceDiagramCytoscapeHandler slug registry assets webConfig logger)
